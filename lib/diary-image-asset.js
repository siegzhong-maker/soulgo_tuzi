...
export async function persistDiaryImageFromDataUrl(dataUrl, options = {}) {
  ...
  if (!stableUrl) {
    ...
    try {
      ...
      // Vercel / Lambda: /var/task is read-only; no S3/R2 → return inline data URL for the client.
      stableUrl = dataUrl;
    }
  }

  if (stableUrl.startsWith('/')) {
    try {
      ...
      // Ignore manifest failures (same read-only FS edge cases).
    }
  }
